...
fun interface DesktopNavidromeSessionOpener {
    suspend fun open(
        request: NavidromeConnectionLoginRequest,
        clearProviderData: Boolean,
    ): DesktopNavidromeSession
}

/**
 * Desktop implementation of Core's provider-session effect.
 *
 * Core supplies explicit connection intent and owns validation, lifecycle, status, and navigation.
 * This adapter restores protected credentials, performs Navidrome/TLS I/O, and publishes the live
 * provider consumed by the rest of the shared product.
 */
class DesktopCoreProviderSessionPort(
    private val mediaSources: MediaSourceRepository,
    private val sessionOpener: DesktopNavidromeSessionOpener,
    ...
    override suspend fun connect(
        ...
        val login = request.toLoginRequest()
        val session = sessionOpener.open(login, plan.clearProviderData)
...
fun desktopNavidromeSessionOpener(
    ...
    nowEpochMillis: () -> Long = DesktopSystemClock::nowEpochMillis,
): DesktopNavidromeSessionOpener = DesktopNavidromeSessionOpener { login, clearProviderData ->
    openProviderConnectionSession(
        request = ProviderConnectionLifecycleRequest(
            ...
            applyTlsDefaults = { NavidromeTls.applyJvmDefaults(it.tlsSettings) },
