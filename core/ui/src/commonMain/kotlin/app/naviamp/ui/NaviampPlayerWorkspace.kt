...
internal fun readableSurfaceColor(colors: NaviampColors) = colors.background.copy(alpha = 0.88f)

/** One bounded reading surface for every host and for either workspace pane. */
@Composable
internal fun NaviampReadableContent(colors: NaviampColors, keepDarkSurface: Boolean = false, content: @Composable () -> Unit) {
    BoxWithConstraints(Modifier.fillMaxSize(), contentAlignment = Alignment.TopCenter) {
        val surface = if (keepDarkSurface || maxWidth >= 900.dp) readableSurfaceColor(colors) else androidx.compose.ui.graphics.Color.Transparent
