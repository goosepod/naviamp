...
One architecture debt family remains and blocks closing this audit: Android still duplicates portable SQLDelight repositories in `AndroidAudioWaveformStore`, `AndroidLibraryIndexStore`, `AndroidLyricsOffsetStore`, `AndroidLyricsSidecarStore`, `AndroidObjectByteStore`, `AndroidPendingProviderActionStore`, `AndroidPlaybackStore`, `AndroidProviderResponseStore`, `AndroidRadioDjPresetStore`, and `AndroidSidecarStatusStore`; `AndroidStorage` composes those copies. `AndroidAudioStore` and `DesktopAudioStore` also combine portable SQL/eviction/download policy with native `File`/`Path` checks even though iOS already proves that `StorageAudioStore` can own the portable portion behind byte/file facts. These are not legitimate platform capabilities. The next ownership slice is to mount `StorageCoreRepositoryCatalog`/`StorageAudioStore` on Android and Desktop, retain only driver/path/atomic-byte/deletion effects in the hosts, delete the duplicate stores, and strengthen the architecture guard so they cannot return.
...
Current foundation: `core:presentation` owns the host-neutral `NaviampCoreStateStore`, exhaustive typed command catalog, one action graph, strict controller composition, and `NaviampCoreApp`. Core controllers own navigation/history, settings and maintenance, settings sync, connection transactions, Home, Search, Library, details, playlists, Internet Radio, standard and Sonic builders, Downloads/offline, and Now Playing/playback. Android, Desktop, and iOS all mount that graph. Required UI callbacks have no silent product no-ops or competing media-row paths, and the router enforces exactly one controller owner. The fake host and all three real hosts consume the same product. `verifyCoreFirstArchitecture` prevents new common platform imports and new host product surfaces; the remaining repository duplication named in the 2026-07-31 re-audit must be extracted and then added to the guard.
