...
Current foundation: `core:presentation` now owns the host-neutral `NaviampCoreStateStore`, an exhaustive typed command catalog for the existing shared shell, and the one action-graph factory that emits those commands. Its strict common router rejects commands without a Core controller. Core controllers now own navigation and detail history, settings and maintenance policy, connection transactions, Home, Search, Library, and album/artist detail loading and presentation—including stale-result protection and nested-artist restoration. This is not yet the completed Core gate: playlists, radio/mixes, Downloads/offline, Now Playing/playback, full composition, required UI contracts, and the fake-host acceptance suite remain before Android or Desktop is adapted to this graph.
