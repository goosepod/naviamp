...
Current foundation: `core:presentation` now owns the host-neutral `NaviampCoreStateStore`, an exhaustive typed command catalog for the existing shared shell, and the one action-graph factory that emits those commands. Its strict common router rejects commands without a Core controller. Core controllers now own navigation and detail history, settings and maintenance policy, connection transactions, Home, Search, Library, album/artist detail loading, playlist browsing and mutations, Internet Radio, and all standard and Sonic builders—including playback intent, queue/save policy, smart-playlist policy, capability validation, stale-result protection, and nested-artist restoration. This is not yet the completed Core gate: Downloads/offline, Now Playing/playback, full composition, required UI contracts, and the fake-host acceptance suite remain before Android or Desktop is adapted to this graph.
