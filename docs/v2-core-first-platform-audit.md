...
Current foundation: `core:presentation` now owns the host-neutral `NaviampCoreStateStore`, an exhaustive typed command catalog for the existing shared shell, and the one action-graph factory that emits those commands. Its strict common router rejects commands without a Core controller, and the first vertical slice now owns navigation, shell overlays, detail history/back behavior, and corresponding state publication. This is not yet the completed Core gate: the remaining feature controllers must populate the store and execute the commands, the UI contracts must become required, and the fake-host acceptance suite must pass before Android or Desktop is adapted to this graph.
