...
- Windows job: Windows runner with Java 21 and Chocolatey. The workflow installs WiX Toolset 3.14 when `candle.exe` is missing, because jpackage needs WiX for MSI/EXE output.
- macOS job: Apple Silicon macOS runner with Java 21. The packaged platform is `macos-arm64` because the repo currently vendors macOS ARM64 BASS libraries.
- Linux desktop job: planned Linux runner with Java 21 and CMake. The packaged platform will be `linux-x64` once the repo vendors Linux x64 BASS libraries.
