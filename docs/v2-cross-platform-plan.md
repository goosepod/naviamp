...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, live playback state, and ordinary queue coordination are active on Android and Desktop
...
- [ ] Move common queue, Now Playing, provider-action, settings, download, and cache coordination into shared owners. `NaviampPlaybackSessionController` owns saved queue/current-target planning and persistence. `NaviampLivePlaybackController` owns the current track or station, queue, progress, and playback status for Android and Desktop through thin Compose-observable host properties. `NaviampPlaybackQueueCoordinator` owns ordinary queue lifecycle and user mutations: start, replacement, restoration, clear, append, play next, removal, selection, move to next, clear upcoming, upcoming shuffle, metadata replacement, and upcoming replacement. Android and Desktop mirror its returned queue into their existing engine controllers. Adjacent/finished-track transitions, specialized radio and sonic-continuation refills, presentation state, and platform audio execution remain with their current owners.
...
- **Last completed item:** Extended `NaviampPlaybackQueueCoordinator` with move-to-next, clear-upcoming, upcoming shuffle/restore, track metadata replacement, and upcoming replacement. Android queue actions and shuffle now use those decisions, and favorite/rating mutations update both the shared queue and app queue-controller mirror without clearing prepared audio. Desktop routes the same UI actions and playlist-engine metadata, shuffle, and upcoming replacement paths through the coordinator. Radio and sonic continuation fetching remain deliberately platform-owned for now.
- **Next recommended item:** Move adjacent selection and finished-track queue transitions into the shared coordinator, including next, previous, repeat behavior, removal of played history, and the queue snapshot used for engine commands. Keep the actual play/restart/seek calls, BASS prepared-next execution, and Android foreground-service callbacks in platform adapters.
