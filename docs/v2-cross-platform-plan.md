...
- **Last completed item:** `DesktopApplicationServicesFactory` now assembles Desktop settings-sync, cache, maintenance, and download services behind one typed host composition boundary. `DesktopNaviampApp` retains only the live Compose state adapters needed by that graph and is down from 1,682 to 1,646 lines. The nineteen delete-first Desktop corrections remain at 4,553 net lines removed because this composition-root split is tracked as structure rather than deletion.
- **Next recommended item:** Consolidate the remaining connection and controller effects into one explicit host-effects boundary, then reassess the large controller-construction section for the next behavior-owning extraction.
