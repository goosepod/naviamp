...
- **Last completed item:** The first delete-first correction moved reusable action-source resolution and its tests from Desktop into `core:ui`, deleted two redundant Desktop-only resolver helpers, and reduced `desktopMain` by 97 net production lines.
- **Next recommended item:** Centralize resolved domain-track and media-item dispatch in shared action handlers, then delete the repeated action switches from `DesktopSharedContentActions`; require the slice to reduce `desktopMain` by at least 150 additional production lines.
