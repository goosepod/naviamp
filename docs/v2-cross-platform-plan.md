...
- [x] Move common session initialization into the shared runtime. Android and Desktop now replace their direct one-shot auto-connect effects with thin session adapters and enter the same once-only runtime gate. `NaviampConnectionController` now owns restoration-source selection, duplicate-attempt gating, fresh-versus-restored cleanup policy, and common connection lifecycle state through the shared application controller graph. Hosts still construct providers and perform HTTP, persistence, library refresh, and playback restoration I/O.
...
- **Last completed item:** `NaviampConnectionController` now owns restoration-source selection, fresh-versus-restored attempt policy, duplicate-attempt gating, and common connection state. It is part of `NaviampApplicationControllers`/`NaviampApplicationRuntime` and is wired through both Android and Desktop connection lifecycles; provider creation, HTTP/TLS, persistence, library refresh, and playback restoration execution remain host/provider integrations.
- **Next recommended item:** Replace the remaining duplicated Android and Desktop connection/restoration presentation fields with observation of the shared connection state, then introduce dependency-aware application assembly for the provider-action, settings, cache, and download controllers. Keep Android service ownership explicit while reducing the large host composition functions to platform adapters and callbacks.
- **Verification:** Shared tests cover restoration-source selection, fresh/restored cleanup plans, duplicate connection-attempt gating, connection state, and inclusion in the application controller graph. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers, verifying both host adapters and the Apple-compatible shared controller.
- **Known blockers:** None.
