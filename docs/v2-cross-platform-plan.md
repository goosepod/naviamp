...
- **Last completed item:** Desktop downloads now pass `NaviampDownloadsScreenUi` into a panel that renders shared downloaded-track, download-job, and offline-dashboard models directly. `DownloadJob.toDownloadJobUi()` owns progress labels and action availability, while `DesktopDownloadsRoute` retains downloaded-file lookup and repository callbacks. Native download-folder selection remains correctly isolated in Desktop settings.
- **Next recommended item:** Convert the Desktop Internet Radio screen from raw `InternetRadioStation` rendering to a grouped shared screen model. Keep provider refresh/save/delete and stable-ID resolution in a focused Desktop adapter while moving row presentation and status across the shared boundary.
