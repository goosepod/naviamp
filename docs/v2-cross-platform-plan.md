...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] Audit every shared UI action callback for hidden Android/Desktop policy divergence. The detail-back defect proved that a common callback type does not guarantee common behavior when each host supplies an independent lambda. Compare the complete `NaviampAppShellActions` graph and focused detail/Now Playing/settings action factories side by side; move routing, validation, selection, and state-transition decisions into common commands/controllers, retain only native execution and lifecycle effects in hosts, and add parity tests for every divergence found. Amend the earlier final ownership audit rather than assuming callback shape establishes shared ownership.
