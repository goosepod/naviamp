...
- **Verification:** On 2026-07-20, `:core:ui:jvmTest`, `:apps:desktop:desktopTest`, `:apps:android:compileDebugKotlin`, and `:core:ui:compileKotlinIosSimulatorArm64` passed after moving route-product composition into common UI. A Desktop smoke test then exposed an infinite-height constraint when Downloads' `LazyColumn` was nested in the route wrapper's vertical scroll; Downloads now explicitly owns scrolling and the route policy has a JVM regression test.
- **Known blockers:** None.
