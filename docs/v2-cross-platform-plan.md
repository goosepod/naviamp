...
- [ ] Add shared error reporting and capability-aware UI behavior. `NaviampApplicationStatusController` provides one typed, observable owner for runtime, connection, pending-provider-action, cache-maintenance, download, and application-wide settings-sync outcomes. Android and Desktop consume the same sequenced status events, while folder selection, cancellation, progress, and form guidance remain page-local. Runtime failures retain structured operation details without swallowing coroutine cancellation. `NaviampCapabilityPresentation` now converts the host capability registry into shared visible, enabled, and experimental decisions. Android and Desktop declare tested capability matrices instead of all-unavailable placeholders; Downloads navigation, Android update checks, and Android settings import/export entry points consume those decisions. Continue applying the policy to certificate controls, sharing, and future iOS surfaces.
- [ ] Test the shared application controller independently of all three platform hosts. Lifecycle/bootstrap, controller composition, navigation/playback relationships, settings sync, and extracted product coordinators have common tests on JVM and iOS simulator; continue extending end-to-end fake-service flows as owners move into `core:app`.
...
- **Last completed item:** Android and Desktop now supply explicit tested platform-capability matrices. The shared runtime exposes `NaviampCapabilityPresentation`, and the first presentation consumers hide unsupported Downloads navigation, update checks, and settings import/export entry points without checking an operating-system name. Runtime fake-service coverage also verifies capability exposure and foreground connectivity refresh.
- **Next recommended item:** Apply the shared capability policy to TLS certificate controls and any remaining host-specific file, sharing, and update surfaces. Then add a test composition that combines `NaviampApplicationRuntime`, `NaviampApplicationControllers`, and `NaviampApplicationServices` with fakes so connection retry, provider-action replay, cache maintenance, downloads, and settings synchronization can be exercised from one application graph.
