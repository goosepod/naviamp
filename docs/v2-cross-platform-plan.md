...
- **Last completed item:** `DesktopPlaybackController` no longer constructs `PlaybackQueueManager`. Next-button eligibility and queue-index validation use `NaviampPlaybackQueueCoordinator`; Desktop retains stopped-state reporting and native previous, next, and jump execution.
- **Next recommended item:** Audit the remaining Desktop previous/next/jump command dispatch for a narrow shared navigation-command owner. Preserve Desktop coroutine scope, BASS/playlist-engine execution, stopped-state reporting, and Android's service-lifetime transition path.
