...
- **Last completed item:** Shared media-source persistence replaces the duplicated Android and Desktop stores. Credential protection is a narrow adapter: Android retains Keystore encryption and automatic plaintext migration, Desktop explicitly uses pass-through storage, and iOS can provide Keychain later. Settings-sync mirror/provider/path implementations now share one document-store contract without moving URI permissions or native dialogs out of their hosts. Presentation convergence has started with shared connection and capability shell groups replacing the corresponding flat Android-only state.
- **Next recommended item:** Make Desktop construct and consume `NaviampShellConnectionUi` and `NaviampShellCapabilitiesUi`, then introduce shared grouped models for library/search/content state to shrink `DesktopAppRouteContent` and the remaining Android shell container. Retain playback engines, native window state, and host lifecycle callbacks as adapters.
