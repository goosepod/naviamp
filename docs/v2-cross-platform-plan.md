...
- [ ] Move remaining shared product behavior out of the Desktop entry point and Desktop-only controller. `NaviampPlaybackQueueCommandController` now owns the common mutate-then-mirror sequence for move-next, remove, and empty-queue user commands. Desktop delegates those entry-point actions through its playback adapter, while Android consumes the same shared command owner; each host retains only playback-engine queue application and prepared-next/native callback handling. `NaviampPlaybackSessionController` now also owns position-delta persistence gating and time-based save throttling. Desktop no longer keeps the last persisted position in Compose state, and Android no longer keeps the last session-save timestamp in `AndroidAppState`; both hosts supply current session inputs while the shared owner decides and records when persistence is due.
...
- **Last completed item:** Playback-session position-delta gating and timed save throttling now belong to `NaviampPlaybackSessionController`. Desktop and Android provide current session inputs and retain their repositories, clocks, and playback callbacks without storing duplicate gate state in their host application state.
- **Next recommended item:** Audit shuffle ownership next. Desktop and Android still keep separate shuffled-up-next snapshots and host-specific toggle/mirror sequences even though `NaviampPlaybackQueueCoordinator` already exposes shared shuffle planning; preserve playback-engine queue application and native prepared-next invalidation as host execution.
