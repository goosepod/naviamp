...
- **Last completed item:** `NaviampPlaybackNavigationCommandController` now resolves and dispatches Desktop previous, next, restart-current, and valid queue-jump commands. Desktop retains stopped-state reporting, coroutine scope, and native playlist-engine execution; Android's behaviorally distinct adjacent/service path is unchanged.
- **Next recommended item:** Move Desktop's direct queue-to-session mapping out of `DesktopPlaybackController`. The shared playback-session owner should accept the queue and position and construct the saved session, while Desktop retains the settings repository and callback timing.
