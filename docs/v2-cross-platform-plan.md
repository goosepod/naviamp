...
- [ ] Add shared error reporting and capability-aware UI behavior. `NaviampApplicationStatusController` provides one typed, observable owner for runtime, connection, pending-provider-action, cache-maintenance, download, and application-wide settings-sync outcomes. Android and Desktop consume the same sequenced status events, while folder selection, cancellation, progress, and form guidance remain page-local. Runtime failures retain structured operation details without swallowing coroutine cancellation. `NaviampCapabilityPresentation` converts the host registry into shared visible, enabled, and experimental decisions. Android and Desktop declare tested capability matrices instead of all-unavailable placeholders; Downloads navigation, Android update checks, settings import/export entry points, file selection, insecure TLS verification, custom server certificates, and client certificates consume those decisions. Sharing and future iOS surfaces remain.
- [ ] Test the shared application controller independently of all three platform hosts. `NaviampApplicationComposition` now combines the runtime/controller graph and dependency-aware services exactly as Android and Desktop do. A complete fake composition drives lifecycle restoration, capability presentation, settings export, cache maintenance, and download-job state on the JVM and iOS simulator; existing focused tests cover connection retry and provider-action replay. Extend the complete composition flow through those provider paths before marking this item complete.
...
- **Last completed item:** Capability-aware presentation now covers insecure TLS verification, custom server certificates, client certificates, and file-backed settings sync in addition to Downloads and updates. Android and Desktop construct one `NaviampApplicationComposition`; a complete fake composition proves lifecycle, settings export, cache maintenance, download jobs, shared controllers, and capabilities are reachable from that same graph.
- **Next recommended item:** Extend the complete fake composition through connection failure/retry and provider-action replay. Audit remaining sharing and file-picker entry points before closing the capability-aware presentation item.
