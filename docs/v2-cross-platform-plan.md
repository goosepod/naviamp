...
| Files and storage locations | `PlatformCapability.FileSelection` presentation boundary; narrow I/O contracts pending | App-owned locations plus settings document/tree picker | Native folder adapter for settings, cache, and downloads | Host adapter pending |
...
- **Last completed item:** Capability-aware presentation and complete-composition testing are closed for the existing Android and Desktop hosts. The fake application composition covers restoration retry, connection retry, provider-action replay, settings export, cache maintenance, downloads, shared controllers, and capabilities on JVM and iOS simulator targets. Every current file-picker and sharing entry point has been audited; native dialogs remain thin-host adapters.
- **Next recommended item:** Continue the platform-services inventory by extracting narrow secret and file I/O contracts where shared application owners actually need them. Then move the remaining host presentation state into shared screen models before beginning the iOS shell.
- **Verification:** `:core:domain:jvmTest`, `:core:app:jvmTest`, `:core:ui:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, `:core:app:iosSimulatorArm64Test`, and `:core:ui:compileKotlinIosSimulatorArm64` pass together with at most two Gradle workers.
- **Known blockers:** None.
