...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] Close every shared UI action ownership gap identified by the Android/Desktop parity audit. The detail-back defect proved that a common callback type does not guarantee common behavior when each host supplies an independent lambda; the completed inventory now defines the migrations required before this parent outcome can close.
    ...
    - [ ] Move album, artist, playlist-detail, media-row, and smart-playlist selection/action routing into exhaustive common dispatchers. Correct the confirmed Desktop action omissions through those owners rather than adding more Desktop-only lambdas.
...
- **Last completed item:** The full shared-action callback parity audit is recorded in [V2 Shared Action Parity Audit](v2-shared-action-parity-audit.md). It identifies confirmed Desktop no-ops, ambiguous callback paths, hidden cross-host policy, legitimate host boundaries, and the required common presentation-composition seam.
- **Next recommended item:** Define the required common commands and narrow execution ports, remove silent no-op defaults for required product actions, and correct the confirmed album/artist/playlist/Now Playing omissions through common dispatchers. Then build the common presentation composition and finish the physical-device matrix before Milestone 5.
- **Verification:** This audit was source-based and changed documentation only. Every action contract was traced from its shared composable call site to Android and Desktop construction; `git diff --check` passed. The preceding shared detail-navigation implementation remains covered by `:core:app:allTests`, `:apps:desktop:desktopTest`, `:apps:android:testDebugUnitTest`, and `:apps:android:compileDebugKotlin`.
