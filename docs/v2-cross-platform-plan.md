...
- **Last completed item:** Desktop library and search now render `NaviampLibraryScreenUi`, `NaviampSearchScreenUi`, `SharedMediaItemUi`, and `SharedTrackRowUi` directly. Shared track rows carry the compact rating label needed to preserve Desktop metadata presentation, while a tested focused adapter resolves stable IDs into existing domain action targets. The older domain Desktop artist/album row wrappers now delegate to the same shared row implementations instead of maintaining duplicate presentation code.
- **Next recommended item:** Convert Desktop album and artist detail panels to consume the existing `SharedAlbumDetailUi` and `SharedArtistDetailUi` models already used by Android. Group detail loading/error state with those models, then isolate the remaining domain-object action resolution as narrowly as library/search now do. After that, apply the same pattern to playlist and download screens.
