...
- [ ] Move remaining shared product behavior out of the Desktop entry point and Desktop-only controller. `NaviampPlaybackQueueCommandController` now owns the common mutate-then-mirror sequence for move-next, remove, and empty-queue user commands. Desktop delegates those entry-point actions through its playback adapter, while Android consumes the same shared command owner; each host retains only playback-engine queue application and prepared-next/native callback handling. `NaviampPlaybackSessionController` now also owns position-delta persistence gating, time-based save throttling, and queue-to-session mapping. Desktop no longer keeps the last persisted position in Compose state, maps queues into settings sessions, or keeps a save timestamp; Android no longer keeps the last session-save timestamp in `AndroidAppState`. Both hosts supply current session inputs while the shared owner decides, maps, and records persistence. The shuffled-up-next snapshot and toggle/restore decision now belong to shared live playback state and `NaviampPlaybackQueueCoordinator`; Android and Desktop retain active-queue normalization and native queue application without maintaining independent shuffle snapshots. `NaviampPlaybackRepeatCommandController` owns the common cycle-then-mirror sequence; Desktop and Android apply the selected shared repeat mode only to their Compose adapters and native queue controllers. Next-button availability and queue-index validation now also resolve through the shared queue coordinator, removing `DesktopPlaybackController`'s private `PlaybackQueueManager`. `NaviampPlaybackNavigationCommandController` now resolves and dispatches Desktop previous, next, restart-current, and queue-jump intent; Desktop retains coroutine scope and native playlist-engine execution. Android deliberately retains its adjacent-selection path because queue-repeat wrapping, Sonic Autoplay fallback, and finished-track service behavior have a distinct lifetime and policy boundary. Initial Now Playing report eligibility now also resolves through `NaviampPlaybackReportingController` for Desktop, Android UI, and Android service lifetimes. `DesktopPlaybackReportingAdapter` now isolates Desktop provider lookup, offline-capable execution, dispatcher selection, and best-effort coroutine handling from `DesktopPlaybackController`; the injected shared reporting owner remains authoritative for eligibility, mapping, and throttling. The delete-first correction moved detail, playlist, and Internet Radio action-source resolution into tested shared `SharedActionSources`, removed redundant Desktop-only resolver helpers, and reduced `desktopMain` by a net 97 production lines without relocating code to another Desktop file. Shared resolved track and media-item dispatch now owns stable-ID lookup and action policy; Desktop supplies host execution callbacks. Removing the superseded domain-model row wrappers and fallback policy from `DesktopMediaRows` reduced `desktopMain` by another net 400 production lines. The unreferenced legacy `DesktopConnectionPanel` product surface was deleted after repository-wide call-site verification, and recent-track dispatch now uses the shared resolved handler, removing another net 205 Desktop production lines. Desktop settings now render the shared settings product surface; the 297-line host wrapper retains native directory selection and Desktop storage-location adaptation while another net 1,373 Desktop production lines were removed. Shared `NaviampNowPlayingContentInput` and `NaviampNowPlayingPresentationInput` now map complete track, radio, queue, lyrics, waveform, visualizer, capability, playlist, full-player, and compact-player state into one renderable contract used by both Android and Desktop. `DesktopPlayerRouteContent` is a 25-line renderer receiving only colors, presentation, and actions; the Desktop full-player and compact-player panels were deleted, while engine/action adaptation is isolated in `DesktopNowPlayingShell`. `DesktopShellChrome` now composes the shared compact player with Desktop bottom navigation and owns tested detail-route normalization. The complete Now Playing and shell-chrome correction removes 198 net Desktop production lines and 11 net Android production lines. `DesktopAppShellStateFactory` and `DesktopAppShellActionFactory` separate shared model/action assembly from the composition root without claiming deletion credit. `NaviampProductRouteContent` now renders home, library, search, downloads, playlists, details, radio, and all mix-builder routes from common UI; the 44-line Desktop wrapper supplies only its native settings slot, removing another net 325 Desktop production lines.
...
- [ ] Adapt Desktop database, secret, filesystem, connectivity, and HTTP services to the shared platform contracts. Media-source SQL, mapping, and credential policy use `StorageMediaSourceStore`; Desktop deliberately supplies its pass-through credential adapter until OS-backed secure storage is implemented. `StorageDatabaseDriverFactory` now isolates platform driver creation, and Desktop's JVM adapter owns directory creation, JDBC loading, SQLite URL construction, busy timeout, and WAL connection configuration while schema creation, migrations, and compatibility repairs remain in database assembly. Settings-sync reads and writes use the shared document-store boundary while paths and native dialogs remain in the host. `DesktopConnectivityMonitor` supplies synchronous shared snapshots from active, non-loopback JVM network interfaces and preserves Desktop's prior online-first behavior when interface inspection is unavailable. `DesktopRuntimeErrorReporter` maps structured shared lifecycle failures to the Desktop process error stream through a testable host sink.
...
- **Last completed item:** Shared shell-state and shell-action factories now keep product mapping and action assembly out of `DesktopNaviampApp`, which is down to 1,818 lines. `NaviampProductRouteContent` renders every non-settings product route from `core:ui`; the 44-line Desktop wrapper supplies the native settings slot. This third correction removes another net 325 Desktop production lines, bringing the nineteen delete-first Desktop corrections to 4,553 net lines removed.
- **Next recommended item:** Audit the remaining `DesktopNaviampApp` lifecycle and effect regions, beginning with connection/settings-sync orchestration and the large controller-construction section. Extract only cohesive lifetimes or move genuine cross-platform policy into shared owners; keep BASS, native dialogs, window behavior, and OS services in the host. After each extraction, reassess whether the remaining composition root can be expressed as a small set of remembered controller graphs plus host effects.
- **Verification:** On 2026-07-20, `:core:ui:jvmTest`, `:apps:desktop:desktopTest`, `:apps:android:compileDebugKotlin`, and `:core:ui:compileKotlinIosSimulatorArm64` passed after moving route-product composition into common UI. Desktop smoke testing then exposed infinite-height constraints where Downloads and playlist details were nested inside the route wrapper's vertical scroll even though both screens own scrolling. The wrapper now adds scrolling only for Search, Playlists, and Internet Radio, with JVM regression coverage for the route policy.
- **Known blockers:** None.
