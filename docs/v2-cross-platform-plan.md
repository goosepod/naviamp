...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, live playback state, queue coordination, generated queue application, seek planning, pending seek state, and the first playback-execution contract are active on Android and Desktop
...
- [ ] Define a platform-services container and narrow interfaces for every platform dependency. `NaviampPlatformServices` now covers capabilities, session lifecycle, playback-session persistence, seek/replay execution, connectivity, and error reporting. Android and Desktop playback controllers implement the narrow execution adapter while retaining BASS and platform lifecycle ownership; shared code owns seek planning and pending seek state before dispatching the host operation. Add secrets, files, and other OS integrations only as their owners are extracted.
...
- **Last completed item:** Moved seek request planning and pending seek state into shared playback code. `NaviampPlaybackCommandController` now reads the shared live playback snapshot, rejects unsupported radio seeks, computes direct-seek versus replay-current behavior, publishes the new progress and pending seek state atomically, and dispatches only the narrow host execution command. Android and Desktop now provide seek inputs and mirror the shared result for UI refresh. The previous-button restart threshold is centralized at 3 seconds in common domain code.
- **Next recommended item:** Move play, pause, resume, and stop command decisions through the shared playback command boundary while keeping BASS calls, Android foreground-service ownership, and prepared audio lifecycle in the host adapters.
