...
- **Last completed item:** `NaviampPlaybackSessionController.saveQueue` now owns queue-to-session mapping. Desktop supplies queue, position, and callback timing while its settings repository remains the persistence adapter.
- **Next recommended item:** Move initial Now Playing report eligibility into `NaviampPlaybackReportingController`. Desktop and Android currently repeat the same capability and Internet Radio decision before invoking their distinct provider-action executors; Android's offline enqueue and service lifetimes must remain intact.
