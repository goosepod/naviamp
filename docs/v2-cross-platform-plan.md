...
- **Last completed item:** `DesktopAppShellStateFactory` owns shared shell-model construction, and `DesktopAppShellActionFactory` now owns product action assembly plus playlist, detail, and radio action-source snapshots. `DesktopNaviampApp` retains mutable Compose settings updates, controller lifetimes, native dialog state, and OS integrations, but no longer contains the individual home, search, library, playlist, detail, radio, download, or mix-builder action wiring. The composition root is down to 1,818 lines. The eighteen delete-first Desktop corrections remain at 4,228 net lines removed because these logical file splits are tracked as structure rather than deletion.
- **Next recommended item:** Extract the shared route-product renderer from `DesktopAppRouteContent` into `core:ui`, using a narrow host slot for Desktop settings. This should remove Desktop-only duplication for home, library, search, downloads, playlists, details, radio, and mix-builder routes while keeping native settings integration in the host. Then reevaluate the remaining `DesktopNaviampApp` lifecycle/effect regions for the next cohesive boundary.
- **Verification:** On 2026-07-20, `:apps:desktop:compileKotlinDesktop` and `:apps:desktop:desktopTest` passed after both Desktop shell-state and shell-action extraction checkpoints. The preceding shared UI, Android, and iOS simulator gate remains green.
- **Known blockers:** None.
