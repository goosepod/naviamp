...
- **Last completed item:** The unreferenced 196-line `DesktopConnectionPanel` legacy product surface was deleted after the shared shell replacement was verified as its only active successor. Recent-track stable-ID lookup and dispatch in `DesktopNaviampApp` now use the shared resolved handler. This slice reduced `desktopMain` by 205 net production lines; the three delete-first corrections total 702 net lines removed.
- **Next recommended item:** Audit the active `DesktopSettingsPanel` boundary: retain native directory pickers and Desktop-specific presentation, but remove any superseded shared settings product UI or policy. Continue to require repository-wide call-site evidence and a measured net Desktop reduction.
- **Verification:** On 2026-07-19, `:apps:desktop:desktopTest` and `:core:ui:jvmTest` passed after the legacy connection-panel deletion and recent-track dispatch change. The preceding combined `:core:ui:jvmTest`, `:core:ui:compileKotlinIosSimulatorArm64`, `:apps:desktop:desktopTest`, and `:apps:android:testDebugUnitTest` gate also passed with two Gradle workers. Windows compiled the iOS simulator sources; simulator linking and execution remain a macOS CI responsibility.
- **Known blockers:** None.
