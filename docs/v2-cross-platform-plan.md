...
- [ ] Keep operating-system shell integrations in the Desktop host.
  ...
  - [ ] Resolve the updater boundary: version comparison and presentation may remain shared, but update-check execution currently constructs its HTTP client in common UI instead of using a host capability/service.
...
- **Last completed item:** The Desktop cache/download ownership audit is closed. Post-reconciliation status, download labeling, and refresh intent now use tested shared policy; Desktop's large storage files are verified native SQLDelight/filesystem/repository adapters, while cache, maintenance, download, keep-downloaded, eviction, and status policy remain shared. This checkpoint removes a net 3 Desktop production lines.
- **Next recommended item:** Reduce `DesktopNaviampApp` to composition and host effects, then document each remaining platform-specific state bridge and dependency.
