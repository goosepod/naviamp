...
- **Last completed item:** `NaviampApplicationControllers` now requires the pending-provider-action repository and owns the resulting `NaviampProviderActionController`. Android UI connection, reporting, heartbeat, favorite, and metadata flows plus Desktop playback and media flows reuse that graph-owned instance. Android foreground-service playback owns one separate service-lifetime controller, preserving background independence while eliminating per-operation controller construction.
- **Next recommended item:** Add a shared structured application-status/error controller and route connection plus provider-action messages through it before moving more callback-heavy settings, cache, and download construction. Once shared status has an observable owner, introduce a second dependency-aware application-services assembly created after host state exists; use it to group settings sync and maintenance controllers without adding platform callbacks or nullable services to `NaviampApplicationControllers`.
- **Verification:** Shared tests cover provider-action graph identity, explicit offline enqueue policy, offline-capable provider wrapping, derived connection presentation, and the existing runtime/controller relationships. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers; Android compilation verifies both the UI graph owner and separate service owner.
- **Known blockers:** None.
