...
- **Last completed item:** The first three open Milestone 4 ownership audits are complete. Settings-sync host workflow, connection/download preflight decisions, service playback-session throttling, progress/seek planning, and repeat-aware prepared-next work now use common tested owners. `NaviampAndroidApp` fell from 1,038 to 847 lines, and Android production source fell from the 20,407-line entry snapshot to 20,236 lines without counting relocated Android code as shared reduction.
- **Next recommended item:** Audit `AndroidAutoBrowseController` and Android Auto command routing. Keep MediaBrowser/MediaSession APIs, stable IDs, pagination, vehicle safety limits, and service execution in Android while moving portable media-ID selection, voice-query interpretation, catalog selection, and queue intent into shared tested policies where they also serve other hosts.
- **Verification:** On 2026-07-20, `:core:domain:allTests`, `:core:app:allTests`, `:apps:desktop:compileKotlinDesktop`, and `:apps:android:testDebugUnitTest` passed after the three ownership items. The earlier Android debug assembly and four-ABI BASS package verification remain valid for the entry audit; rerun them after the remaining Milestone 4 migrations.
- **Known blockers:** None.
