...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, and live playback state are active on Android and Desktop
...
- [ ] Move common queue, Now Playing, provider-action, settings, download, and cache coordination into shared owners. `NaviampPlaybackSessionController` owns saved queue/current-target planning and persistence. `NaviampLivePlaybackController` now owns the current track or station, queue, progress, and playback status for Android and Desktop through thin Compose-observable host properties. Queue mutation decisions, presentation state, and platform audio execution remain with their current owners.
...
- **Last completed item:** Added `NaviampLivePlaybackController` to `NaviampApplicationRuntime` as the canonical live snapshot for current track, current station, queue, progress, and playback status. Android's existing app-state properties and Desktop's existing local variables now transparently read and write that shared owner, preserving all current callbacks and BASS execution paths.
- **Next recommended item:** Move the first bounded queue mutations—play-next insertion, append, removal, and current-index selection—behind a shared coordinator that updates `NaviampLivePlaybackController`, while keeping actual play/seek/stop commands in the platform engines.
- **Verification:** `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, and `:apps:desktop:desktopTest` passed together. `:core:app:iosSimulatorArm64Test` also compiled and ran the shared live-state tests successfully. Checks ran with at most two Gradle workers.
- **Known blockers:** None.
