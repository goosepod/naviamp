...
- [ ] Add shared error reporting and capability-aware UI behavior. `NaviampApplicationStatusController` now provides one typed, observable owner for runtime, connection, pending-provider-action, cache-maintenance, and download outcomes. Android and Desktop consume the same sequenced status events, runtime failures retain structured operation details without swallowing coroutine cancellation, and offline-action replay, cache-maintenance, plus download status policy is shared. Page-local status remains with its feature owner, and capability-aware shared UI behavior is still pending.
...
- **Last completed item:** `NaviampApplicationServices` now owns the observable download-job controller and download coordinator alongside settings/cache services. Android and Desktop download controllers receive the same assembled instances, Desktop exposes the injected job state rather than maintaining a second list, and application-wide download outcomes publish through shared status while Downloads-page state remains available.
- **Next recommended item:** Consolidate settings-sync status publication where it is application-wide while leaving folder-picker and page validation text local. Then audit the remaining direct `Naviamp*Controller` construction in Android and Desktop and move any application-lifetime owners into the appropriate controller graph or dependency-aware service assembly.
- **Verification:** Shared tests cover service/download assembly identity, observable injected download jobs, typed status sequencing and clearing, graph identity, connection/error publication, runtime retry clearing, offline enqueue/replay policy, and the existing runtime/controller relationships. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers.
- **Known blockers:** None.
