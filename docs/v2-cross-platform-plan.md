...
- **Last completed item:** Desktop BASS now fully implements the audited shared playback contract, including an idempotent native-release boundary invoked only by the Desktop process-shutdown path.
- **Next recommended item:** Extract Desktop SQLite driver construction and connection pragmas from `DesktopCache` into a focused JVM database-driver factory behind a shared storage contract; keep schema creation and migration policy in storage/database assembly.
- **Verification:** On 2026-07-19, `DesktopBassPlaybackEngineContractTest` covers observable stop behavior and idempotent native backend release. The prior combined `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` gate passed with two Gradle workers. Windows compiled the iOS simulator sources and tests; simulator linking and execution remain a macOS CI responsibility. The broader Milestone 2 verification also remains green for `:core:storage:jvmTest`, `:core:ui:jvmTest`, `:core:storage:compileKotlinIosSimulatorArm64`, and `:core:ui:compileKotlinIosSimulatorArm64`.
- **Known blockers:** None.
