...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] After ownership is correct, split the remaining large Android files by logical host responsibility and compose them from focused adapters. Do not create another monolithic composition root or merely relocate code between Android files.
    ...
    - [ ] Revisit the large radio, playlist, media-action, and BASS files only after the final ownership audit identifies stable seams; prefer common coordinators plus small Android execution/state adapters over Android utility-file shuffling.
  - [ ] Run a final Android-versus-Desktop product-ownership audit and remove obsolete Android-only surfaces before closing this item.
    ...
    - [ ] Re-audit Android-prefixed radio, playlist, media, connection, and settings operations against their Desktop callers and common owners after the logical decomposition, removing obsolete wrappers and moving any remaining reusable decisions to common code.
    - [ ] Confirm no Android-only product feature or setting is missing from the shared UI/runtime contract, and record intentional platform-only surfaces such as Android Auto, notifications, permissions, storage volumes, and background-service behavior.
...
- **Last completed item:** The foreground-service composition item is complete, database setup uses the shared driver-factory boundary, and encrypted settings credentials are isolated from ordinary preferences. `AndroidPlaybackForegroundService` is down to 1,682 lines, `AndroidStorage` to 795, and `AndroidSettingsStore` to 606. Total Android production source is 20,129 lines, down 278 from the Milestone 4 entry snapshot.
- **Next recommended item:** Complete the Android filesystem/HTTP ownership audit, then separate filesystem/audio-byte operations from `AndroidStorage`. After that, revisit the large radio, playlist, media-action, and BASS files using the final Android-versus-Desktop ownership audit.
- **Verification:** On 2026-07-20, `:core:domain:jvmTest` and `:apps:android:testDebugUnitTest` passed for the Android Auto ownership work. The earlier complete shared suites, Desktop compile, Android debug assembly, and four-ABI BASS package verification remain valid for the entry audit; rerun the full gate after the remaining Milestone 4 migrations.
- **Known blockers:** None.
