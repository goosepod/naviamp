...
- **Last completed item:** Initial Now Playing report eligibility now belongs to `NaviampPlaybackReportingController` across Desktop, Android UI, and Android service lifetimes. Each host retains provider actions, offline enqueue, coroutine dispatch, and error handling.
- **Next recommended item:** Extract Desktop's provider-report execution and coroutine scheduling from `DesktopPlaybackController` into a focused host reporting adapter. Keep the injected shared reporting owner authoritative for eligibility, mapping, and throttling; do not merge Android UI and service lifetimes.
- **Verification:** On 2026-07-19, the playback-session gating slice passed `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` together with two Gradle workers. Windows compiled the iOS simulator sources and tests; simulator linking and execution remain a macOS CI responsibility. The broader Milestone 2 verification also remains green for `:core:storage:jvmTest`, `:core:ui:jvmTest`, `:core:storage:compileKotlinIosSimulatorArm64`, and `:core:ui:compileKotlinIosSimulatorArm64`.
- **Known blockers:** None.
