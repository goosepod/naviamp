...
- [ ] Add shared error reporting and capability-aware UI behavior. `NaviampApplicationStatusController` now provides one typed, observable owner for runtime, connection, pending-provider-action, and cache-maintenance outcomes. Android and Desktop consume the same sequenced status events, runtime failures retain structured operation details without swallowing coroutine cancellation, and offline-action replay plus cache-maintenance status policy is shared. Page-local status remains with its feature owner, and capability-aware shared UI behavior is still pending.
...
- **Last completed item:** `NaviampApplicationServices` now groups required settings-sync, cache-settings, and cache-maintenance controllers after each host has initialized its adapters. Android settings maintenance and Desktop library/cache settings reuse those instances instead of constructing private owners, and cache-clear outcomes publish through shared application status.
- **Next recommended item:** Add download job/coordinator ownership to the dependency-aware service assembly and inject it into Android and Desktop download controllers, preserving host filesystem and durable-background execution adapters. Then consolidate settings-sync status publication where it is application-wide while leaving folder-picker and page validation text local.
- **Verification:** Shared tests cover service-assembly identity, typed status sequencing and clearing, graph identity, connection/error publication, runtime retry clearing, offline enqueue/replay policy, and the existing runtime/controller relationships. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers.
- **Known blockers:** None.
