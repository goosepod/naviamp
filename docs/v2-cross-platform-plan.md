...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, live playback state, queue coordination, generated queue application, and the first playback-execution contract are active on Android and Desktop
...
- [ ] Define a platform-services container and narrow interfaces for every platform dependency. `NaviampPlatformServices` now covers capabilities, session lifecycle, playback-session persistence, seek/replay execution, connectivity, and error reporting. Android and Desktop playback controllers implement the narrow execution adapter while retaining BASS and platform lifecycle ownership; add secrets, files, and other OS integrations only as their owners are extracted.
...
- **Last completed item:** Added `NaviampPlaybackExecution` to `NaviampPlatformServices` and introduced `NaviampPlaybackCommandController` as the shared dispatcher for seek plans. Android and Desktop playback controllers are thin execution adapters: direct seeks remain engine calls, while transcoded-stream seeks replay the current track at the requested position. Previous-button restart continues through the same seek plan. BASS, prepared audio, Android foreground-service ownership, and lifecycle behavior remain platform-owned.
- **Next recommended item:** Move seek planning and its pending-progress state updates into a shared playback coordinator so hosts provide inputs and execute the returned command instead of duplicating the orchestration. Resolve the existing Previous-button threshold mismatch during that move: Android currently uses 3 seconds and Desktop 10 seconds; `12.5` in the execution test is only arbitrary seek data, not a threshold.
- **Verification:** `:core:app:jvmTest`, `:apps:android:compileDebugKotlin`, and `:apps:desktop:compileKotlinDesktop` passed together. `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` then passed together. Checks used at most two Gradle workers.
- **Known blockers:** None.
