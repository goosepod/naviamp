...
- [ ] Add shared error reporting and capability-aware UI behavior. `NaviampApplicationStatusController` now provides one typed, observable owner for runtime, connection, and pending-provider-action outcomes. Android and Desktop consume the same sequenced status events, runtime failures retain structured operation details without swallowing coroutine cancellation, and offline-action replay status policy is shared. Page-local status remains with its feature owner, and capability-aware shared UI behavior is still pending.
...
- **Last completed item:** `NaviampApplicationStatusController` is graph-owned and publishes typed, sequenced runtime, connection, and provider-action outcomes. Android and Desktop observe it directly; pending-action replay wording and severity are shared, Desktop now replays pending actions after connecting like Android, and a successful runtime retry clears the prior runtime error.
- **Next recommended item:** Introduce a second dependency-aware application-services assembly after host state exists. Use it to group settings sync, cache settings, and maintenance controllers without adding platform callbacks or nullable services to `NaviampApplicationControllers`; then migrate Android and Desktop construction to that assembly one service family at a time.
- **Verification:** Shared tests cover typed status sequencing and clearing, graph identity, connection/error publication, runtime retry clearing, offline enqueue/replay policy, and the existing runtime/controller relationships. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers.
- **Known blockers:** None.
