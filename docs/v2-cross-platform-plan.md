...
- [x] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
...
- [ ] Complete the Android build and launch gate.
  ...
  - [ ] Install the current build on a connected device, launch it, and record the device/API level used for the Milestone 4 acceptance pass. The only target visible on 2026-07-20 was a Pixel 8 emulator (`sdk_gphone64_arm64`, API 34). Installation was intentionally left incomplete because its existing `app.naviamp.android` has a different signing identity; both normal and configured release-key debug installation were rejected with `INSTALL_FAILED_UPDATE_INCOMPATIBLE`, and the existing app/data was not uninstalled.
...
- **Last completed item:** Android and Desktop BASS execution now share one tested request/callback/generation coordinator, and the focused Android service-lifecycle suite covers ownership, sticky restart, restored queue/position, system-surface republishing, and UI reattachment. All stable Android product-policy seams identified by the final ownership audit are now shared. Total Android production source is 20,184 lines, down 223 from the Milestone 4 entry snapshot; the increase from the previous checkpoint is the explicitly tested service-lifecycle boundary rather than relocated product policy.
- **Next recommended item:** Complete the Milestone 4 physical-device matrix, starting with an install and launch on the Pixel 10a, then exercise background/service restoration, system media controls, storage/document access, downloads, and Android Auto. Do not begin Milestone 5 until the Android acceptance findings are recorded or explicitly deferred.
- **Verification:** On 2026-07-20, all configured `core:domain`, `core:app`, `core:ui`, `core:storage`, and `providers:navidrome` test targets passed, along with `:apps:android:testDebugUnitTest`, debug assembly, and four-ABI BASS package verification. Android and Desktop compilation also passed for the common BASS migration.
- **Known blockers:** The Pixel 10a was not connected. The available Pixel 8 API 34 emulator contains a differently signed Naviamp package, so installing the current APK would require deleting that app and its data; this was not done implicitly.
