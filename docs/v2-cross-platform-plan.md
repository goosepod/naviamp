...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, live playback state, and bounded queue mutations are active on Android and Desktop
...
- [ ] Move common queue, Now Playing, provider-action, settings, download, and cache coordination into shared owners. `NaviampPlaybackSessionController` owns saved queue/current-target planning and persistence. `NaviampLivePlaybackController` owns the current track or station, queue, progress, and playback status for Android and Desktop through thin Compose-observable host properties. `NaviampPlaybackQueueCoordinator` now owns the first bounded user mutations—append, play next, removal, and current-index selection—and both hosts mirror its returned queue into their existing engine controllers. Queue lifecycle and continuation mutations, presentation state, and platform audio execution remain with their current owners.
...
- **Last completed item:** Added `NaviampPlaybackQueueCoordinator` to the shared runtime and routed Android and Desktop append, play-next, removal, and current-index selection through it. The coordinator reuses the existing domain queue rules and atomically updates `NaviampLivePlaybackController`; each host still mirrors the result into its current queue controller so prepared-next invalidation, callbacks, BASS preparation, and play/seek/stop execution do not change. Specialized radio and sonic-continuation mutations remain in their current owners.
- **Next recommended item:** Move queue lifecycle decisions—start or replace a queue, restore it, and clear it—behind the shared coordinator, beginning with normal track playback and saved-session restoration while preserving Android foreground-service ownership and Desktop BASS callbacks.
- **Verification:** `:core:app:jvmTest` passed the shared coordinator tests. `:apps:android:compileDebugKotlin` and `:apps:desktop:compileKotlinDesktop` passed together, followed by `:apps:android:testDebugUnitTest` and `:apps:desktop:desktopTest`. `:core:app:iosSimulatorArm64Test` also compiled and ran the coordinator tests successfully. Checks used at most two Gradle workers.
- **Known blockers:** None.
