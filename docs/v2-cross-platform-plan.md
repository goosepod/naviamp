...
- **Status:** Milestone 2 in progress; shared startup, controller composition, navigation, playback/queue coordination, reporting, offline provider actions, settings sync, cache settings, and download execution are active on Android and Desktop
...
- [ ] Move common queue, Now Playing, provider-action, settings, download, and cache coordination into shared owners. `NaviampPlaybackSessionController` owns saved queue/current-target planning and persistence. `NaviampLivePlaybackController` owns the current track or station, queue, progress, pending seek state, playback status, and repeat mode for Android and Desktop through thin Compose-observable host properties. `NaviampPlaybackQueueCoordinator` owns ordinary queue lifecycle, user mutations, generated queues, Sonic Autoplay application, and the previous-button threshold. `NaviampPlaybackCommandController` owns transport, volume, and seek decisions. `NaviampPlaybackReportingController` and the shared heartbeat runner own reporting eligibility, mapping, session/state deduplication, throttling, and cancellation-safe heartbeat timing for Android Activity playback, Android service-owned playback, and Desktop. `NaviampProviderActionController` owns offline-capable Now Playing and favorite mutations plus pending-action replay; hosts retain provider I/O scheduling. `NaviampSettingsSyncController` owns local snapshot construction, timestamps, import/export, and startup reconciliation. `NaviampCacheSettingsController` owns cache-setting normalization, publication, persistence, and ordered platform application. `NaviampDownloadJobController` and `NaviampDownloadCoordinator` own stable jobs, execution routing, progress, cancellation/retry intent, and keep-downloaded reconciliation; hosts retain files, network/background execution, status presentation, and storage-location selection. Cache maintenance, presentation state, BASS, and remaining platform lifecycle work still need to converge further.
...
- **Last completed item:** Moved settings snapshot/sync orchestration and download execution/keep-downloaded reconciliation into shared application controllers. Android and Desktop now supply platform stores and repositories to those owners. Added `NaviampApplicationControllers` so every host receives one coherent navigation, playback, and queue graph; both current hosts construct and pass that bundle to the shared runtime.
- **Next recommended item:** Move the remaining sync-file selection and settings status workflow behind shared contracts, then extract cache-maintenance and download status/error presentation policy. Continue reducing the large Android and Desktop composition functions by grouping provider/session and media-feature controllers behind shared application assembly without moving BASS or operating-system lifecycle ownership prematurely.
- **Verification:** Shared tests cover controller-graph identity/queue ownership and settings snapshot export/import in addition to the existing lifecycle and product-controller contracts. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:compileDebugKotlin`, and `:apps:desktop:compileKotlinDesktop` pass together. `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` also pass together with at most two Gradle workers.
- **Known blockers:** None.
