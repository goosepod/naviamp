...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] Close every shared UI action ownership gap identified by the Android/Desktop parity audit. The detail-back defect proved that a common callback type does not guarantee common behavior when each host supplies an independent lambda; the completed inventory now defines the migrations required before this parent outcome can close.
    ...
    - [ ] Define required common action commands and narrow execution ports. Remove silent no-op defaults from required product actions, represent optional behavior through explicit capabilities, and eliminate competing direct/generic callback paths.
      ...
      - [ ] Apply the same required-versus-capability-explicit contract rule to the remaining shell, detail, media, settings, and Now Playing action groups.
    ...
    - [ ] Move media-row and smart-playlist selection/action routing into exhaustive common dispatchers. Correct the confirmed Desktop action omissions through those owners rather than adding more Desktop-only lambdas. Playlist, album, and artist detail routing are complete in the contract slices above.
      ...
      - [ ] Remove the legacy broad media request enum, conversion boundary, and UI-local permissive handler tables by emitting sealed commands directly from shared rows and dialogs.
    ...
    - [ ] Add a common presentation-composition module or dependency arrangement that consumes the shared application graph and shared UI without reversing the existing `core:app` and `core:ui` dependency direction.
    - [ ] Construct the shared UI state and action graph from that common composition root instead of requiring each host to rebuild per-feature callback wiring.
    ...
    - [ ] Add required-action completeness tests, Android/Desktop parity contracts, and a host-neutral composition test that browses Home, Search, Library, album, artist, playlist, Downloads, radio, and Settings using fake platform services.
...
- **Last completed item:** Host-facing media commands are sealed by kind, media product capabilities are defined once in common UI, and arbitrary artist catalog loading is shared so Android and Desktop expose the same artist-row actions. Smart-playlist operations and stale-source failure are required common contracts too.
- **Next recommended item:** Make shared rows and dialogs emit the sealed media commands directly, delete the legacy enum/conversion and permissive UI handlers, then drive the common action catalog from explicit capabilities so unsupported actions cannot be emitted.
- **Verification:** Shared UI tests plus Android, Desktop, and iOS simulator production compilation passed for the required media and smart-playlist contract migration.
