...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] After ownership is correct, split the remaining large Android files by logical host responsibility and compose them from focused adapters. Do not create another monolithic composition root or merely relocate code between Android files.
    ...
    - [ ] Compose `AndroidPlaybackForegroundService` from focused notification, MediaSession/MediaBrowser, Android Auto selection, and service-lifecycle adapters. Keep the service itself as the OS entry point and measure both service-file reduction and net Android source so relocation is visible.
      ...
      - [ ] Extract MediaSession metadata/queue/playback-state publication and MediaBrowser trust/callback plumbing into focused composed Android adapters.
...
- **Last completed item:** Notification presentation is now composed into `AndroidPlaybackForegroundService` through `AndroidPlaybackNotificationFactory`; service lifetime, command authorization, refresh timing, and cover-art loading remain in the service. The service is 2,186 lines and total Android production source is 20,239 lines after recording the composition overhead, still down from the 20,407-line Milestone 4 entry snapshot.
- **Next recommended item:** Continue the service checklist by extracting MediaSession metadata/queue/state publication and MediaBrowser trust/callback plumbing into focused Android adapters, then separate Auto selection execution from service lifecycle. Complete database/filesystem contracts before decomposing storage and settings.
