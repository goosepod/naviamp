...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] Move detail-page history and back policy into the shared application navigation owner. A Pixel 10a acceptance test found that the shared album/artist UI exposed one `onBack` interface while Android and Desktop supplied independent host implementations; Android had incorrectly wired both buttons to close Now Playing even though its system-back adapter called the correct detail controller. The immediate Android fix routes both UI buttons through `AndroidNavigationController.closeActiveDetail` and is covered for album and nested-artist behavior. The convergence follow-up should make shared UI and Desktop call the same common detail-navigation command, with Android system back remaining only an OS adapter.
...
- **Last completed item:** Android and Desktop BASS execution now share one tested request/callback/generation coordinator, and the focused Android service-lifecycle suite covers ownership, sticky restart, restored queue/position, system-surface republishing, and UI reattachment. The Pixel 10a build/launch gate also exposed and fixed an Android detail-button adapter bug while identifying shared detail-navigation ownership as one remaining convergence seam. Total Android production source is 20,184 lines before this small acceptance fix, down 223 from the Milestone 4 entry snapshot; the increase from the previous checkpoint is the explicitly tested service-lifecycle boundary rather than relocated product policy.
- **Next recommended item:** Verify the corrected album and artist UI back actions on the installed Pixel 10a, then move detail history/back policy into the shared application navigation owner. Continue the physical-device matrix with background/service restoration, system media controls, storage/document access, downloads, and Android Auto before Milestone 5.
- **Verification:** On 2026-07-20, all configured `core:domain`, `core:app`, `core:ui`, `core:storage`, and `providers:navidrome` test targets passed, along with `:apps:android:testDebugUnitTest`, debug assembly, and four-ABI BASS package verification. Android and Desktop compilation also passed for the common BASS migration. The signed release then installed and cold-launched successfully on a Pixel 10a running API 37 with no immediate fatal/error log.
- **Known blockers:** None. The remaining Milestone 4 work is hands-on physical-device behavior verification.
