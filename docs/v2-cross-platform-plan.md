...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] After ownership is correct, split the remaining large Android files by logical host responsibility and compose them from focused adapters. Do not create another monolithic composition root or merely relocate code between Android files.
    ...
    - [ ] Revisit the large radio, playlist, media-action, and BASS files only after the final ownership audit identifies stable seams; prefer common coordinators plus small Android execution/state adapters over Android utility-file shuffling. The audit identified the following independently measurable seams:
      ...
      - [ ] Compose the 864-line Android and 883-line Desktop BASS engines around a tested common execution coordinator where their state machine matches. Keep Android audio focus, wake locks, TLS/native paths, JNI/backend calls, and Desktop device/native-library integration in their hosts.
...
- **Last completed item:** Shared coordinators now own seeded-radio continuation/build/expansion sequencing and playlist playback transactions, and Android uses the same shared row/download action dispatchers as Desktop. Android's radio, playlist, and media-action files are now 799, 763, and 557 lines respectively; Desktop's corresponding radio, playlist, and metadata-action files are 505, 614, and 201. Total Android production source is 20,156 lines, down 251 from the Milestone 4 entry snapshot.
- **Next recommended item:** Compose the Android and Desktop BASS engines around a tested common execution coordinator while preserving Android audio focus/wake locks and each host's native backend integration. Then add the focused foreground-service restoration tests and begin the physical-device acceptance pass.
- **Verification:** On 2026-07-20, `:core:app:jvmTest`, `:core:ui:jvmTest`, `:apps:desktop:compileKotlinDesktop`, and `:apps:android:compileDebugKotlin` passed for the radio, playlist, and media-action migrations. The earlier Android unit suite, complete shared suites, Android debug assembly, and four-ABI BASS package verification remain valid; rerun the full gate after the BASS migration.
- **Known blockers:** None.
