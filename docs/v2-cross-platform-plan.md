...
- **Last completed item:** `DesktopSettingsSyncHost` now owns Desktop document-folder I/O, status policy, import/export, auto-export, startup reconciliation, and settings-save hooks around the shared `NaviampSettingsSyncController`. `DesktopNaviampApp` retains Compose state and document-application callbacks but is down from 1,818 to 1,682 lines. The nineteen delete-first Desktop corrections remain at 4,553 net lines removed because this host-lifetime split is tracked as structure rather than deletion.
- **Next recommended item:** Extract construction of `NaviampApplicationServices` and its cache/download/settings-sync service graph into a focused remembered Desktop factory. Then consolidate the remaining connection and controller effects into one explicit host-effects boundary before reassessing the large controller-construction section.
