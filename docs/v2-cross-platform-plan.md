...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] After ownership is correct, split the remaining large Android files by logical host responsibility and compose them from focused adapters. Do not create another monolithic composition root or merely relocate code between Android files.
    ...
    - [ ] Split `AndroidAutoBrowseController` into catalog loading and Android `MediaBrowserCompat` presentation once shared selection ownership is stable; do not leak Android media items into common code.
...
- **Last completed item:** The Android Auto ownership audit is complete. Voice/search interpretation, fuzzy matching, queue-page planning, and catalog playback intents are common and tested; Android retains stable media IDs, URI encoding, browse safety limits, MediaBrowser/MediaSession APIs, and service execution. The service and Activity fallback now consume the same parsed intent. `AndroidPlaybackForegroundService` is 2,207 lines and Android production source is 20,155 lines, down from the 20,407-line Milestone 4 entry snapshot while counting the new Android codec against that total.
- **Next recommended item:** Continue the explicit logical-decomposition checklist under Milestone 4, beginning with notification and MediaSession/MediaBrowser composition out of `AndroidPlaybackForegroundService`. Then split Auto catalog loading from Android media-item presentation before decomposing storage/settings along their platform-contract seams.
