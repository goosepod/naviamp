...
- **Last completed item:** Android and Desktop now construct and consume the same grouped connection and capability shell models. Library and search query/result/loading state also cross both host boundaries through `NaviampLibraryScreenUi` and `NaviampSearchScreenUi`, shrinking `DesktopAppRouteContent` and the Android shell state without moving BASS, native window state, or lifecycle callbacks into shared UI.
- **Next recommended item:** Convert the Desktop library and search panels to consume the shared media and track models directly, then move their action-target resolution into shared action handlers. Continue grouping the remaining album, artist, playlist, downloads, and settings screen state so hosts pass screen models rather than platform-sized argument lists.
