...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] After ownership is correct, split the remaining large Android files by logical host responsibility and compose them from focused adapters. Do not create another monolithic composition root or merely relocate code between Android files.
    ...
    - [ ] Compose `AndroidPlaybackForegroundService` from focused notification, MediaSession/MediaBrowser, Android Auto selection, and service-lifecycle adapters. Keep the service itself as the OS entry point and measure both service-file reduction and net Android source so relocation is visible.
      ...
      - [ ] Separate Android Auto catalog-selection execution from foreground-service lifecycle and transport ownership without duplicating shared `CatalogPlaybackIntent` policy.
...
- **Last completed item:** MediaSession and MediaBrowser ownership are now composed out of the foreground service. Session state is instance-scoped in `AndroidMediaSessionController`, browser trust/delegation lives in `AndroidMediaBrowserController`, and the service fell from 2,186 to 1,917 lines. Total Android production source is 20,262 lines after recording the 23-line composition overhead, still down from the 20,407-line Milestone 4 entry snapshot.
- **Next recommended item:** Separate Android Auto catalog-selection execution from foreground-service lifecycle while continuing to consume shared `CatalogPlaybackIntent`, then complete the database-driver contract before decomposing storage/settings.
