...
- **Last completed item:** Android and Desktop now observe `NaviampConnectionRuntimeState` directly for connection/restoration presentation. Android's `restoringConnection` and Desktop's `isConnecting` mirrors were removed, Android restoration intent is explicit, and the shared controller graph can be assembled with an existing connection controller or initial connection state for host-independent testing and future iOS composition.
- **Next recommended item:** Introduce dependency-aware application assembly for the provider-action controller first, so Android UI and Desktop reuse one graph-owned instance while Android's foreground service retains its explicit service-owned instance. Then apply the same pattern where it materially reduces settings, cache, and download construction without turning `NaviampApplicationControllers` into a platform-dependent service locator.
- **Verification:** Shared tests cover derived connection presentation, restoration-source selection, fresh/restored cleanup plans, duplicate connection-attempt gating, and controller-graph identity. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers, verifying direct `StateFlow` observation, removal of the host flags, and Apple-compatible controller assembly.
- **Known blockers:** None.
