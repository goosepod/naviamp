...
- **Status:** Milestone 2 in progress; shared startup, controller composition, connection/restoration lifecycle, navigation, playback/queue coordination, reporting, offline provider actions, settings sync/reconciliation, cache settings/maintenance, and download execution are active on Android and Desktop
...
- [ ] Define a platform-services container and narrow interfaces for every platform dependency. `NaviampPlatformServices` now covers capabilities, session lifecycle, playback-session persistence, playback transport execution, connectivity, and error reporting. Android and Desktop playback controllers implement the narrow execution adapter while retaining BASS and platform lifecycle ownership; shared code owns play/pause/resume/stop dispatch, volume command application, seek planning, and pending seek state before dispatching host operations. `StorageCredentialProtector` now isolates credential protection from shared media-source SQL and mapping, while `NaviampSettingsSyncDocumentStore` isolates selected settings-document I/O from Android URI and Desktop path APIs. Add remaining HTTP, database-driver creation, picker, notification, and other OS integrations only as their owners are extracted.
...
- **Last completed item:** Common queue, Now Playing, provider-action, settings-sync, download, and cache coordination now has shared owners. The closing cross-host audit also moved keep-downloaded toggling, policy construction, and collection dispatch into `NaviampDownloadCoordinator` while preserving platform provider fetch and execution adapters.
- **Next recommended item:** Finish the Milestone 2 platform-services container by auditing the remaining direct HTTP, database-driver creation, picker, notification, and OS integrations, adding narrow contracts only where shared owners actually require them. Broader Desktop and Android entry-point reduction remains in Milestones 3 and 4.
