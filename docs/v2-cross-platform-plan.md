...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] After ownership is correct, split the remaining large Android files by logical host responsibility and compose them from focused adapters. Do not create another monolithic composition root or merely relocate code between Android files.
    ...
    - [ ] Revisit the large radio, playlist, media-action, and BASS files only after the final ownership audit identifies stable seams; prefer common coordinators plus small Android execution/state adapters over Android utility-file shuffling. The audit identified the following independently measurable seams:
      ...
      - [ ] Consolidate shared track/media action dispatch now split across the 559-line Android and 201-line Desktop controllers, then compose Android downloaded-track, radio-enqueue, and UI-state adapters without merely relocating them.
...
- **Last completed item:** The Android filesystem/HTTP ownership audit is complete, `AndroidStorage` now composes focused audio-file services, and the final Android-versus-Desktop ownership audit has converted the remaining stable seams into explicit radio, playlist, media-action, and BASS checklist items. `AndroidPlaybackForegroundService` remains at 1,682 lines, `AndroidStorage` is down to 727, and `AndroidSettingsStore` remains at 606. Total Android production source is 20,169 lines, down 238 from the Milestone 4 entry snapshot; the filesystem composition added 40 net host lines while reducing the repository facade by 68.
- **Next recommended item:** Implement the shared radio request/build/expansion coordinator first, then the playlist workflow coordinator and shared media-action dispatch. Defer BASS engine composition until those application workflows are stable because it has the highest native-execution risk.
- **Verification:** On 2026-07-20, `:apps:android:testDebugUnitTest` passed after the audio-file composition, including focused atomic publication, failed-write cleanup, and directory-switch tests. The earlier complete shared suites, Desktop compile, Android debug assembly, and four-ABI BASS package verification remain valid for the entry audit; rerun the full gate after the remaining Milestone 4 migrations.
- **Known blockers:** None.
