...
- **Last completed item:** `DesktopHostEffects` now provides one explicit boundary for application-status mirroring, settings-sync startup, Sonic home discovery, connection-form library lookup, connected-library refresh, and keep-downloaded reconciliation. `DesktopNaviampApp` contains no inline `LaunchedEffect` blocks and is down from 1,646 to 1,542 lines. The nineteen delete-first Desktop corrections remain at 4,553 net lines removed because this host-effect split is tracked as structure rather than deletion.
- **Next recommended item:** Reassess the remaining controller-construction section and extract the next cohesive behavior-owning graph without turning the composition root into a collection of unstructured parameter bags.
