...
- **Last completed item:** Android and Desktop now publish application-wide settings-sync success, warning, and failure outcomes through shared typed status while preserving picker/progress/form text locally. The shared-controller construction audit moved playback-report deduplication into `NaviampApplicationControllers` and documented why execution-bound or service-lifetime controllers must remain separate.
- **Next recommended item:** Add shared capability-aware presentation decisions so unsupported features are hidden or disabled from the same `PlatformCapabilities` input on every host. Extend fake-service runtime tests through at least connection failure/retry, provider-action replay, cache maintenance, downloads, and settings synchronization before marking the Milestone 2 controller-test item complete.
- **Verification:** Shared tests cover graph-owned playback-report identity, service/download assembly identity, observable injected download jobs, typed status sequencing and clearing, connection/error publication, runtime retry clearing, offline enqueue/replay policy, and the existing runtime/controller relationships. `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` pass together with at most two Gradle workers.
- **Known blockers:** None.
