...
- **Last completed item:** Shuffled-up-next snapshot ownership and the common toggle/restore decision now belong to shared live playback state and `NaviampPlaybackQueueCoordinator`. Desktop and Android retain active-queue normalization and native playback-queue application without storing parallel shuffle state.
- **Next recommended item:** Move the common cycle-repeat-then-mirror sequence into a shared command owner. `NaviampPlaybackQueueCoordinator` already owns the repeat-mode decision and live state; Android and Desktop should retain only application to their distinct native queue controllers.
