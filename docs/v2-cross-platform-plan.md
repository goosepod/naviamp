...
- **Last completed item:** Common resolved action handlers now dispatch current domain tracks and media items while Desktop supplies execution callbacks. The obsolete `DesktopArtistRow`, `DesktopAlbumRow`, and `DesktopTrackRow` compatibility wrappers and their fallback product-action switches were unreferenced after the shared-row migration and have been deleted. This slice reduced `desktopMain` by 400 net production lines; the two delete-first corrections total 497 net lines removed.
- **Next recommended item:** Audit the remaining action switches and route-expanded callback construction in `DesktopNaviampApp`, moving reusable product decisions behind existing shared contracts and deleting superseded Desktop glue. Continue to require a measured net reduction rather than file-to-file relocation.
- **Verification:** On 2026-07-19, the combined `:core:ui:jvmTest`, `:core:ui:compileKotlinIosSimulatorArm64`, `:apps:desktop:desktopTest`, and `:apps:android:testDebugUnitTest` gate passed with two Gradle workers. Windows compiled the iOS simulator sources; simulator linking and execution remain a macOS CI responsibility.
- **Known blockers:** None.
