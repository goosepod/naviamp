...
- **Last completed item:** Shared `applySettingsSyncDocument` now performs preference normalization, playback capability filtering, radio-DJ replacement, and server-profile import once for Android and Desktop. `DesktopSettingsSyncDocumentApplicator` retains only Desktop persistence and live Compose-state adaptation. Common mapping tests and both host builds cover the new path, while `DesktopNaviampApp` is down from 1,516 to 1,478 lines. This is a real cross-host policy extraction; the delete-first Desktop total will be recalculated after the next graph checkpoint.
- **Next recommended item:** Isolate the playback and Now Playing controller graph behind cohesive live-state adapters, preserving BASS execution and Compose observation as explicit Desktop boundaries.
