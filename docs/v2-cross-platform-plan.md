...
- **Status:** Milestone 3 complete; Milestone 4 shared-action ownership audit reopened after device testing exposed divergent host behavior behind a shared UI callback
...
- [ ] Move remaining shared product behavior out of the Android UI composition, app controllers, playback service, and Android Auto implementation.
  ...
  - [ ] Audit every shared UI action callback for hidden Android/Desktop policy divergence. The detail-back defect proved that a common callback type does not guarantee common behavior when each host supplies an independent lambda. Amend the earlier final ownership audit rather than assuming callback shape establishes shared ownership.
    - [ ] Compare the complete `NaviampAppShellActions` graph and focused detail, Now Playing, playlist, connection, download, and settings action factories side by side; inventory every callback whose hosts make different product decisions.
    - [ ] Move routing, validation, selection, and state-transition decisions into common commands/controllers. Retain only native execution, platform capability, and lifecycle effects in host adapters, with parity tests for every divergence found.
    - [ ] Construct the shared UI state and action graph from a common application/presentation composition root instead of requiring each host to rebuild per-feature callback wiring.
    - [ ] Add a host-neutral composition test that mounts the common product UI/runtime using fake platform services and can browse its major routes without Android or Desktop controllers.
...
- **Last completed item:** Detail-page return routes and nested-artist history moved into the common `NaviampNavigationController`; Android and Desktop now consume one shared command, and Android system back is only an OS adapter. Migration builds are visibly labeled `v2.0.0-alpha` in About.
- **Next recommended item:** Perform the newly explicit shared-action callback parity audit before claiming Android product ownership is complete. Then continue the physical-device matrix with background/service restoration, system media controls, storage/document access, downloads, and Android Auto before Milestone 5.
- **Verification:** `:core:app:allTests`, `:apps:desktop:desktopTest`, `:apps:android:testDebugUnitTest`, and `:apps:android:compileDebugKotlin` passed after the shared detail-navigation migration. The central `v2.0.0-alpha` version and build 35 also passed version validation and focused Desktop/Android resource generation.
- **Known blockers:** None. Remaining Milestone 4 work includes the shared-action callback parity audit and hands-on physical-device behavior verification.
