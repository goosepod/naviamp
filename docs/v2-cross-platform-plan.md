...
- **Last completed item:** Desktop and the shared shell now render public `NaviampPlaylistsContent` from the complete playlists screen/action and media-action contracts. Shared Playlists owns sorting, explicit refresh, smart-playlist creation/editing, row actions, rename/delete confirmation, and add-to-playlist dialogs. Removing the Desktop list composition and narrowing the remaining file to Playlist Detail reduced `desktopMain` by 221 net production lines; the ten delete-first corrections total 3,616 net lines removed.
- **Next recommended item:** Promote Playlist Detail to its public shared screen/action contracts, then delete the final Desktop playlist composition while preserving bulk tools, standard/smart playlist editing, and complete track/media actions.
- **Verification:** On 2026-07-19, the combined `:core:ui:jvmTest`, `:core:ui:compileKotlinIosSimulatorArm64`, `:apps:desktop:desktopTest`, and `:apps:android:testDebugUnitTest` gate passed after the shared Playlists route migration. Windows compiled the iOS simulator sources; simulator linking and execution remain a macOS CI responsibility.
- **Known blockers:** None.
