...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, live playback state, and queue lifecycle coordination are active on Android and Desktop
...
- [ ] Move common queue, Now Playing, provider-action, settings, download, and cache coordination into shared owners. `NaviampPlaybackSessionController` owns saved queue/current-target planning and persistence. `NaviampLivePlaybackController` owns the current track or station, queue, progress, and playback status for Android and Desktop through thin Compose-observable host properties. `NaviampPlaybackQueueCoordinator` owns bounded append, play-next, removal, selection, and queue lifecycle decisions including start, replacement, restoration, and clear. Android and Desktop mirror its returned queue into their existing engine controllers. Specialized radio and sonic-continuation mutations, remaining reorder/shuffle operations, presentation state, and platform audio execution remain with their current owners.
...
- **Last completed item:** Extended `NaviampPlaybackQueueCoordinator` with start, replacement, restoration, and clear decisions. Android normal track starts, saved-session restoration, reset clearing, and internet-radio queue clearing now update the shared queue before mirroring into the app queue controller. Desktop injects the coordinator into `DesktopPlaylistEngine`, so all normal starts, restores, clears, and explicit replacements pass through the shared owner before existing callbacks and BASS work run. Android foreground-service execution and both platforms' engine session/prepared-next behavior remain unchanged.
- **Next recommended item:** Move the remaining user-visible queue mutations—move to next, clear upcoming, upcoming shuffle, track metadata replacement, and upcoming replacement—through `NaviampPlaybackQueueCoordinator`, leaving radio/sonic continuation refills and adjacent/finished-track execution for a following transition-focused slice.
