...
- **Status:** Milestone 2 in progress; shared startup, navigation, saved sessions, live playback state, queue coordination, generated queue application, seek planning, pending seek state, transport commands, and volume dispatch are active on Android and Desktop
...
- [ ] Move common queue, Now Playing, provider-action, settings, download, and cache coordination into shared owners. `NaviampPlaybackSessionController` owns saved queue/current-target planning and persistence. `NaviampLivePlaybackController` owns the current track or station, queue, progress, pending seek state, playback status, and repeat mode for Android and Desktop through thin Compose-observable host properties. `NaviampPlaybackQueueCoordinator` owns ordinary queue lifecycle and user mutations; repeat-aware previous, next, adjacent, and finished-track transitions; generated-radio stale-result rejection, deduplication, upcoming replacement, append, and history-limit decisions; Sonic Autoplay continuation application; and one shared previous-button restart threshold. `NaviampPlaybackCommandController` now owns play/pause/resume/start-or-restore decisions, stop dispatch, volume clamping and application, seek request planning, shared progress and pending seek updates, and seek versus replay-current selection. Android and Desktop apply shared decisions through their existing engine controllers. Provider fetching, request/session tokens, background execution, presentation state, BASS, and platform lifecycle remain with their current owners.
...
- **Last completed item:** Moved software-volume command application into `NaviampPlaybackCommandController` and widened the narrow host execution adapter with `setVolume`. Android Now Playing and Desktop playback-settings effects now use this shared path. Android sleep-timer expiry now stops through `AndroidPlaybackAppController`, preserving the same forced session save used by other Android stop commands. Android and Desktop still own BASS calls and settings persistence.
- **Next recommended item:** Start extracting playback reporting and provider-action coordination into `core:app`, beginning with the duplicated Now Playing reporting eligibility and heartbeat behavior while leaving provider I/O and host coroutine/lifecycle ownership behind the shared boundary.
- **Verification:** `:core:domain:jvmTest`, `:core:app:jvmTest`, `:apps:android:compileDebugKotlin`, and `:apps:desktop:compileKotlinDesktop` passed together. `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` then passed together. The shared command test verifies supported volume clamping and prevents engine application when software volume is unavailable. Checks used at most two Gradle workers, and Gradle daemons were stopped after verification.
- **Known blockers:** None.
