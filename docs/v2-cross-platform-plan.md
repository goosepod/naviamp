...
- **Last completed item:** `DesktopRestoredAppState` now owns startup reads, saved connection recovery, radio-DJ preset migration, and the immutable restored-state snapshot used to seed the shared runtime. Native-token recovery has focused Desktop regression coverage. `DesktopNaviampApp` is down from 1,542 to 1,516 lines. The nineteen delete-first Desktop corrections remain at 4,553 net lines removed because this startup-assembly split is tracked as structure rather than deletion.
- **Next recommended item:** Extract settings-sync document application from the composition root, then isolate the playback and Now Playing controller graph behind cohesive live-state adapters.
