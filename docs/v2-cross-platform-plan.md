...
- [ ] Adapt the existing Desktop BASS implementation to the shared playback contract. The 2026-07-19 contract audit confirmed that play, pause/resume, source-aware seek, clamped software volume with ReplayGain, and prepared-next queue adoption already implement the shared contract while native polling and end-sync callbacks correctly remain in the host. Desktop BASS `stop()` now publishes `PlaybackState.Stopped` and `PlaybackProgress.Unknown` before clearing callbacks, matching the observable contract used by the shared runtime and Android. Final native backend release at process shutdown remains the verified lifecycle gap; ordinary stop intentionally keeps BASS initialized for later playback.
- [ ] Adapt Desktop database, secret, filesystem, connectivity, and HTTP services to the shared platform contracts. Media-source SQL, mapping, and credential policy now use `StorageMediaSourceStore`; Desktop deliberately supplies its pass-through credential adapter until OS-backed secure storage is implemented. Settings-sync reads and writes use the shared document-store boundary while paths and native dialogs remain in the host. `DesktopConnectivityMonitor` now supplies synchronous shared snapshots from active, non-loopback JVM network interfaces and preserves Desktop's prior online-first behavior when interface inspection is unavailable. `DesktopRuntimeErrorReporter` maps structured shared lifecycle failures to the Desktop process error stream through a testable host sink.
...
- **Last completed item:** The Desktop BASS contract audit is recorded, and `stop()` now publishes the shared stopped-state and unknown-progress reset before clearing host callbacks.
- **Next recommended item:** Add a narrow optional playback-engine release contract and have Desktop invoke it only at process shutdown so BASS native state is freed without changing ordinary stop/restart behavior or Android service ownership.
- **Verification:** On 2026-07-19, the Desktop BASS stop-contract slice passed `DesktopBassPlaybackEngineContractTest`. The prior combined `:core:app:jvmTest`, `:apps:android:testDebugUnitTest`, `:apps:desktop:desktopTest`, and `:core:app:iosSimulatorArm64Test` gate passed with two Gradle workers. Windows compiled the iOS simulator sources and tests; simulator linking and execution remain a macOS CI responsibility. The broader Milestone 2 verification also remains green for `:core:storage:jvmTest`, `:core:ui:jvmTest`, `:core:storage:compileKotlinIosSimulatorArm64`, and `:core:ui:compileKotlinIosSimulatorArm64`.
- **Known blockers:** None.
