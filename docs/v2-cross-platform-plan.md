...
- **Last completed item:** `DesktopAppShellStateFactory` now owns construction of `NaviampAppShellUiState`, including shared settings, home, downloads, mix builders, library, search, detail, playlist, and radio models. `DesktopNaviampApp` supplies current host/controller snapshots without owning their UI mapping. The prior complete Now Playing correction removes 198 net Desktop production lines and 11 net Android production lines; the eighteen delete-first Desktop corrections remain at 4,228 net lines removed because this logical file split is tracked as structure rather than deletion.
- **Next recommended item:** Extract construction of `NaviampAppShellActions` and its shared action-source snapshots from `DesktopNaviampApp`, preserving mutable Compose state updates, native dialogs, controller lifetimes, and OS integrations in the composition root. Then extract the shared route-product renderer from `DesktopAppRouteContent`, retaining only the Desktop settings/native slot in the host.
- **Verification:** On 2026-07-20, `:apps:desktop:compileKotlinDesktop` and `:apps:desktop:desktopTest` passed after extracting Desktop shell-state construction. The preceding shared UI, Android, and iOS simulator gate remains green.
- **Known blockers:** None.
