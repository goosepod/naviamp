...
- `DesktopNaviampApp.kt` is still large at roughly 1,360 lines. Many controllers have been extracted, but reducing this file remains a real incomplete item.
...
- Fourth post-audit reduction slice: local action wrappers moved into `DesktopAppActions`, keeping controller delegates and recent-radio action routing out of `DesktopNaviampApp.kt`.
...
- Ninth post-audit reduction slice: route content rendering moved into `DesktopAppRouteContent`, keeping the main `when(appRoute)` panel selection, route model shaping, and panel callbacks out of `DesktopNaviampApp.kt` while leaving state ownership in the app root.
