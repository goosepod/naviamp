...
| `tokenInfo` / API-key authentication | Shared connection mode selects token/salt, password, or API key. Key-only requests omit `u`, `p`, `t`, and `s`; `tokenInfo` supplies the account name. The key stays in protected local storage and is omitted from settings export. Interoperability with a released compatible Navidrome build is pending. |
