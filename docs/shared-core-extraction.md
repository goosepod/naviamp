...
- Lyrics parsing, embedded/tag lyrics extraction, LRCLIB response parsing, LRCLIB diagnostics sanitizing, and audio tag/ReplayGain parsing now live in shared core. Android and desktop keep only file-probe and HTTP transport adapters around the shared lyrics/audio logic.
- Shared LRCLIB and Deezer HTTP calls now use a Ktor-backed `KtorSharedHttpClient` from `core/domain`; Android and desktop keep only diagnostics wrappers for those clients.
- Navidrome GET/POST/PUT execution now lives in shared `KtorNavidromeHttpClient`; Android and desktop keep only Ktor CIO client construction plus TLS material loading for custom certificates, client certificates, and insecure TLS settings.
- Shared HTTP byte and streaming helpers now live on `KtorSharedHttpClient`. Android/desktop image cache, cover art, notification art, visualizer art, audio cache/download, and internet-radio playlist resolution use Ktor-backed shared code instead of direct `URL.openStream`/`HttpURLConnection` calls; the desktop BASS engine still owns native playback streaming via `BASS_StreamCreateURL`.
