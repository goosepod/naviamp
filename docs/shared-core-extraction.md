...
- Shared LRCLIB and Deezer HTTP calls now use a Ktor-backed `KtorSharedHttpClient` from `core/domain`; Android and desktop keep only diagnostics wrappers for those clients. Navidrome HTTP still uses its existing platform adapters until the Ktor migration can preserve custom TLS certificates, client certificates, and insecure TLS behavior with focused tests.
